...
st.markdown(
    "**Deep dive into a pharmaceutical company's 2022 commercial footprint** — "
    "where they invested, with whom, and on what."
)
...
selected_label = st.selectbox("Select Company", companies["label"].tolist())
...
c4.metric("Avg Payment Size", f"${kpis['avg_payment'].iloc[0]:,.0f}")
...
with col1:
    ...
    fig = bar_chart(
        ...
        title=f"{selected_company} — Top 10 Specialties by Spend ($K)",
        orientation="h",
    )
...
with col2:
    ...
    fig = donut_chart(
        ...
        title=f"{selected_company} — Payment Type Mix",
    )
    st.plotly_chart(fig, use_container_width=True)
...
fig = choropleth_us(
    ...
    title=f"{selected_company} — 2022 Spend by State ($K)",
)
st.plotly_chart(fig, use_container_width=True)
...
st.subheader("Top 25 Physicians Paid")
